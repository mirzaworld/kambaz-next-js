...
export default function EditPostScreen({
  ...
  const [postType, setPostType] = useState<"QUESTION" | "NOTE">(postToEdit?.type || "QUESTION");
  const [visibility, setVisibility] = useState<"ENTIRE_CLASS" | "SELECTED_STUDENTS">(
    postToEdit?.visibility || "ENTIRE_CLASS"
  );
  const [selectedFolders, setSelectedFolders] = useState<string[]>(postToEdit?.folders || []);
  const [summary, setSummary] = useState(postToEdit?.summary || "");
  const [details, setDetails] = useState(postToEdit?.details || "");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [courseUsers, setCourseUsers] = useState<any[]>([]);
  const [selectedUserIds, setSelectedUserIds] = useState<string[]>(postToEdit?.visibleToUserIds || []);
  ...
  useEffect(() => {
    const loadFolders = async () => {
      try {
        ...
        if (response.ok) {
          const foldersList = await response.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${SERVER_URL}/api/courses/${courseId}/pazza/posts/${postToEdit._id}`, {
        method: "PUT",
